						...
						<table class="table table-striped table-sm">
							...
							<tbody v-for="(article,index) in t" :key="index">
								<tr>
									...
									<td>{{article.state}}</td>
									<td>
                                        <button type="button" class="btn btn-primary" @click="shencha(article.studentid)">审查</button>
                                    </td>
								</tr>
							</tbody>
						</table>
...
export default {
  ...
  methods: {
	  ...
	  shencha(id){
		 window.location.href="http://127.0.0.1:8080/user/shtp?studentid="+id;
	  },
	  updateTheUser(id){
        ...
        getMerchantInformation(val){   
            ...
            var l = 0; //总长度
            this.$axios.get("http://127.0.0.1/user/getAllPay?start="+val)
            .then((response) => {
  ...
  mounted(){
    ...
    this.checkinsin();
		//获取数据 
		this.$axios.get("http://127.0.0.1/pay/number")
		.then(res => {
			 this.end = res.data;
		})
